...
const createEvent = asyncHandler(async (req, res) => {
    try {
        ...
        eventData.employeesRequested = employeesRequested;
        eventData.employees = employees;
        eventData.pendingStaff = employeesRequested.length;
        eventData.totalStaff = employeesRequested.length;
        ...
        try {
            ...
            await Promise.allSettled(staffUsers.filter(staff => staff.email).map(staff =>
        ...
        try {
            const userId = req.userId || event.created_by;
            await createNotificationHendlar(userId, 1, `Your event "${event.name_title || 'Event'}" has been created successfully.`, userId);
...
const getAllEvents = asyncHandler(async (req, res) => {
    try {
        const { page = 1, limit = 10, search = '', status, event_type_id, Event_type, country_id, state_id, city_id, event_category_tags_id, user_id, min_price, max_price, budget_min, budget_max, DateRange, sortBy = 'created_at', sortOrder = 'desc' } = req.query;
        const filter = {};
        if (search) {
            filter.$or = [
...
const checkPaymentStatus = asyncHandler(async (req, res) => {
    try {
        ...
        if (!paymentResult.success) {
            return sendSuccess(res, { payment_status: 'error', error_message: paymentResult.error, payment_intent: null, message: 'Payment verification failed' }, 'Payment status checked with error', 200);
        }
        const { paymentIntent } = paymentResult;
        if (paymentIntent.status === 'succeeded') {
            const transaction = await Transaction.findOne({ reference_number: paymentIntent.id, });
            if (!transaction) {
            ...
            if (updatedTransaction) {
                sendSuccess(res, { payment_status: 'succeeded', payment_intent: { id: paymentIntent.id, status: paymentIntent.status, amount: paymentIntent.amount / 100, currency: paymentIntent.currency, created: paymentIntent.created }, transaction: populatedTransaction, message: 'Payment verified and confirmed successfully' }, 'Payment verified and confirmed successfully', 200);
            }
        } else {
            sendSuccess(res, { payment_status: paymentIntent ? paymentIntent.status : 'unknown', payment_intent: paymentIntent ? { id: paymentIntent.id, status: paymentIntent.status, amount: paymentIntent.amount / 100, currency: paymentIntent.currency } : null, message: 'Payment status checked but not succeeded' }, 'Payment status checked', 200);
        }
    } catch (error) {
...
module.exports = { createEvent, getAllEvents, getEventById, updateEvent, deleteEvent, getEventsByAuth, getEventsExcludingAuth, eventPayment, getAllStaffEvents, rejectStaffEvent, acceptStaffEvent, getEventInviteMemberById, checkPaymentStatus };