...
const createStaffEventBook = asyncHandler(async (req, res) => {
  try {
    ...
    const allreadyStaffEventBook = await StaffEventBook.findOne({ event_id: req.body.event_id, staff_id: req.body.staff_id, });
    if (allreadyStaffEventBook) {
      let staffPrice = null, initial_payment = allreadyStaffEventBook.initial_payment;
      if (allreadyStaffEventBook.staff_id) {
        const priceDoc = await StaffWorkingPrice.findOne({ staff_id: allreadyStaffEventBook.staff_id, status: true });
        staffPrice = priceDoc ? priceDoc.price : null;
        if (allreadyStaffEventBook.initial_payment == (null || undefined)) {
          ...
          allreadyStaffEventBook.actualAmount = priceDoc.price;
          allreadyStaffEventBook.initial_payment = initial_payment;
          allreadyStaffEventBook.pendingPayment = priceDoc.price - baseAmount;
          allreadyStaffEventBook.platform_fee = customerPlatformFeeAmount;
          allreadyStaffEventBook.initialPerPayment = baseAmount;
          await allreadyStaffEventBook.save();
        }
      }
      const response = {
        ...allreadyStaffEventBook.toObject(),
...
const StaffBookingPayment = asyncHandler(async (req, res) => {
  try {
    const { staff_event_book_id, payment_method_id, billingDetails, description = 'Staff initial booking payment' } = req.body;
    ...
    const staffWorkingPrice = await StaffWorkingPrice.findOne({ staff_id: staffEventBook.staff_id, status: true });
    ...
    let customerId = null;
    try {
      const customerData = { email: user.email, name: user.name, phone: user.mobile, metadata: { user_id: req.userId, staff_event_book_id: staff_event_book_id } };
    ...
    try {
      const paymentOptions = { amount: totalAmount, billingDetails: billingDetails, currency: 'usd', customerEmail: user.email, metadata: { user_id: req.userId, customer_id: customerId, staff_event_book_id: staff_event_book_id, payment_type: billingDetails, description: description } };
...
const venueBookingPaymentByUser = asyncHandler(async (req, res) => {
  try {
    const { event_id, venue_details_id, payment_method_id, billingDetails, description = 'Final Venue payment' } = req.body;
    if (!event_id || !payment_method_id) {
      return sendError(res, 'event_ids and payment_method_id are required', 400);
    }
    const staffEventBook = await Event.findOne({ event_id: parseInt(event_id) });
    if (!staffEventBook) {
      return sendNotFound(res, 'Staff event booking not found');
    }
    const findVenueDetails = await VenueDetails.findOne({ venue_details_id: parseInt(venue_details_id) });
    if (!findVenueDetails) {
    ...
    try {
      const customerData = {
        ...
        phone: user.mobile,
        metadata: {
          user_id: findVenueDetails.createdBy,
          event_id: event_id,
    ...
    if (totalAmount > 0) {
      const staffTransactionData = {
        user_id: req.userId, // Staff user ID
        amount: totalAmount, // Staff receives: baseAmount - 7% platform fee
        status: paymentIntent.status,
        payment_method_id: parseInt(payment_method_id, 10),
        transactionType: 'StaffBooking',
    ...
    const updatedStaffEventBook = await Event.findOneAndUpdate(
      { event_id: parseInt(event_id) },
      {
        transaction_id: customerTransaction.transaction_id,
        transaction_status: 'Completed',
