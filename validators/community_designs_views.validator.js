...
/**
 * Community Designs Views validation schemas using Joi
 */
...
// Create community design view validation schema
const createCommunityDesignViewSchema = Joi.object({
  community_designs_id: commonValidations.community_designs_id,
  status: commonValidations.status
});

// Update community design view validation schema
const updateCommunityDesignViewSchema = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .required()
    .messages({
      'number.base': 'Community Designs Views ID must be a number',
      'number.integer': 'Community Designs Views ID must be an integer',
      'number.positive': 'Community Designs Views ID must be a positive number',
      'any.required': 'Community Designs Views ID is required'
    }),
...
// Get community design view by ID validation schema
const getCommunityDesignViewByIdSchema = Joi.object({
  id: Joi.number()
    .integer()
    .positive()
    .required()
...
// Get all community design views query validation schema
const getAllCommunityDesignViewsSchema = Joi.object({
  page: Joi.number()
    .integer()
    .min(1)
    .default(1)
